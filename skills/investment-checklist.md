...
1. **估值锚**：说明采用的利润口径、总股本、对应归母利润/Owner Earnings。
...
3. **买入纪律**：表格后必须用一句话给出行动区间，例如：
   - "`XX元`以下是安全边际区；`XX-YY元`是观察/小仓位区；`YY元`以上需要增长继续兑现。"
   - 如果当前价格高于保守买入锚，必须直说"当前不便宜/不适合买入"，不能只给中性描述。

4. **估计值标注**：凡是股本、EPS、汇率、正常化利润、恢复期利润、分部利润来自推算，必须标注"估计"和低/中/高置信度。
