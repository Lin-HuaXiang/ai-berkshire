...
description: 供应链瓶颈猎手：从超级趋势出发，寻找产业链物理瓶颈和套利机会。不问"推荐什么股票"，问"如果这个趋势继续扩张，哪一环会先不够用？"当用户提到"瓶颈"、"供应链"、"bottleneck"、"卡脖子"、"短缺"、"supply chain"时使用。
...
**瓶颈真实不等于投资机会。** 必须对每家公司计算PS（市销率）、PE（市盈率）并标注在报告中。以下用组合条件判断估值是否透支：
...
2. **PS 是同行盈利公司的5倍以上**：必须解释溢价来源（市场份额、增速差、壁垒差异）
3. **PE > 80x**：需要计算PEG并说明增速是否支撑
...
**必填字段**：市值、年收入、PS、PE为必填项，不可用"待核实"跳过。如果无法获取财务数据，信号强度不得超过★★。
...
关键数据：市值$XX / 年收入$XX / PS Xx / PE Xx / 收入增速XX% / 瓶颈业务占比XX%
...
- `{趋势名}-bottleneck-{YYYYMMDD}.md` — 每次完整扫描的独立报告
...
**财务快照**：市值$XX / 年收入$XX / PS Xx / PE Xx / 收入增速XX%
**估值检查**：红灯/黄灯/绿灯（具体说明）
...
7. **小市值不等于好机会** — 小市值也可能是烂公司，必须过财务质量关
8. **瓶颈真实不等于投资机会** — 一家公司可以坐在最紧的瓶颈上，但如果PS>30x或仍在亏损，当前价格就不是买点。**估值是硬门槛，不可被瓶颈纯正度、信号强度或叙事吸引力覆盖。** 宁可错过一个涨了的瓶颈股，也不要在100倍PS买入一家亏损公司
