...
description: 管理层纵深研究。对指定公司的关键管理层进行深度评估：诚信度、战略眼光、资本配置能力、治理结构、侧面验证。当用户提到"管理层分析"、"CEO评估"、"管理团队"、"老板靠不靠谱"、"management deep dive"时使用。
tools: Read,Write,Bash,WebSearch,WebFetch
---

# 管理层纵深研究：买股票就是买人

对 $ARGUMENTS 进行管理层深度研究。本 Skill 采用单 Agent 顺序执行。
...
本Skill是投资研究管理层评估的**深化版**。当标准投资研究中管理层评分不确定（★★★或以下）、或管理层是核心投资逻辑时，使用本Skill做纵深研究。
...
确认关键人物后，依次使用 WebSearch 和 WebFetch 收集以下数据：
1. CEO公开发言与预测记录（股东信、电话会、采访、社交媒体）
2. 资本配置决策记录（并购、回购、分红、新业务投资）
3. 治理结构与薪酬（股权结构、关联交易、高管薪酬）
4. 侧面验证信息（员工评价、客户反馈、行业口碑）
...
|------|--------------|---------|:------:|
| | "我们认为X市场会..." | X市场实际... | 正确/错误 |
| | "未来3年我们的重点是..." | 实际执行... | 正确/错误 |
...
|---|------|---------|---------|---------|------|
| 1 | | "我们将在2025年实现X业务盈利" | 2024年报电话会 | | 兑现/部分/未兑现 |
| 2 | | "我们计划回购$X亿" | 2024年股东信 | | 兑现/部分/未兑现 |
...
将报告写入 `reports/{TICKER}/{TICKER}-management-{YYYYMMDD}.md`（TICKER为英文名/股票代码，禁止中文），例如 `reports/Meituan/Meituan-management-20260409.md`
