...
description: 巴菲特-芒格-段永平-李录四大师综合投资研究框架。对指定公司进行系统化的深度投资研究，覆盖生意本质、护城河、逆向思考、管理层、行业趋势、估值、综合决策七大模块。当用户提到"研究XX公司"、"深度分析XX"、"XX股票怎么样"、"investment research"、"投研"时使用。
tools: Read,Write,Bash,WebSearch,WebFetch
---

# 投资研究：巴菲特-芒格-段永平-李录 四大师综合分析框架

对 $ARGUMENTS 进行系统化投资研究分析。本 Skill 采用单 Agent 顺序执行，依次完成前置评估、数据收集、生意分析、护城河评估、逆向思考、管理层评估、行业趋势、估值分析和综合决策。
...
使用 WebSearch 和 WebFetch 从网络收集以下数据：
...
8. **风险因素**：地缘政治、监管、供应链等
9. **当前估值**：市值、PE（市盈率）、PS（市销率）、PEG、EV/Revenue（企业价值/收入）
10. **多空双方核心论点**
...
5. 如果工具报告偏差过大，必须排查原因后才能继续分析

**常见错误防范**：
- 市值单位：港币亿 vs 人民币亿 vs 美元亿，容易漏写/多写一个零
- FCF（自由现金流）口径：不同来源对资本支出的定义可能不同（是否含租赁、收购等）
- 债务口径：是否包含经营租赁负债
- 持股比例：AB股公司的经济权益不等于投票权
...
- 用一句话定义这门生意的本质
- 收入结构拆解（近3-5年分部收入表格）
- 5年盈利能力趋势（收入/毛利率/经营利润率图表）
- 商业模式画布：一次性销售 vs 订阅/复购？硬件 vs 软件 vs 平台？
- 生态粘性/客户锁定强度
- 毛利率水平与同行对比，解释为什么高/低
- 经营杠杆分析：收入增长1%带来多少利润增长？
...
| 护城河类型 | 验证方法 |
|-----------|---------|
| 品牌/定价权 | 是否能在不损失销量的情况下提价？过去3年提价记录？ |
| 转换成本 | 客户迁移到竞品的成本有多高？数据/关系/习惯迁移成本？ |
| 网络效应 | 用户越多产品越好吗？是单边还是双边/多边？ |
| 规模效应 | 规模带来的成本优势有多大？单位成本下降趋势？ |
| 技术/专利壁垒 | 技术领先几年？能否被复制？专利数量和质量？ |
...
- 列出"这家公司可能失败的所有路径"（表格：路径/概率/影响程度/触发条件）
...
```
- 与自身历史估值对比（3年/5年PE/PB分位）
- 与同行估值对比

**估值与买入价格**（必须输出）：

1. **当前价格**：小节开头列出当前股价、当前市值、行情日期/时间、币种和来源。如果行情延迟或来自估计，标注"估计/延迟"。说明PE/PB/股息率/FCF Yield（自由现金流收益率）。
2. **估值锚**：说明利润口径、总股本、对应归母利润/Owner Earnings。若处于亏损或异常，标注"TTM PE锚失效/低质量"，改用正常化利润或替代锚。
3. **估值倍数表**（4档+当前价格行）：
...
4. **买入纪律**：明确行动区间，如"XX元以下是安全边际区；XX-YY元是观察/小仓位区；YY元以上需要增长继续兑现"。当前不便宜必须直说。
5. **估计值标注**：推算数据标注"估计"和低/中/高置信度。
...
3. 每个模块末尾必须有对应大师的"追问"
4. 最终将完整报告写入 `reports/{TICKER}/{TICKER}-research-{YYYYMMDD}.md`（TICKER为英文名/股票代码，禁止中文）
