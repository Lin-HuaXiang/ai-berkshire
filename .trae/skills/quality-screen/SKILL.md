---
name: quality-screen
description: 去劣筛选：7条硬指标快速排除非一流公司。支持个股/行业/指数/主题批量筛选，输出通过率排名。当用户提到"筛选"、"去劣"、"quality screen"、"排除烂公司"、"批量筛选"时使用。
...
| # | 指标 | 排除条件 | 衡量的是什么 |
|---|------|---------|-------------|
| 1 | 10年平均ROE（净资产收益率） | <8% | 资本效率——股东的钱能不能跑赢机会成本 |
...
为每家公司使用 WebSearch 搜索以下数据（可按市场分批处理）：
