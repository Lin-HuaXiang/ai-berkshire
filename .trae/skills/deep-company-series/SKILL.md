...
description: 深度公司系列：8篇长文拆解一家公司。从认知重置到决策框架，约12万字完整闭环，适合公众号/视频号系列发布。当用户提到"深度系列"、"deep company"、"看懂XX"、"系列长文"、"8篇拆解"时使用。
tools: Read,Write,Bash,WebSearch,WebFetch,Grep,Glob
---

# 深度公司系列：8 篇长文拆一家公司

为 $ARGUMENTS 撰写一个 8 篇深度长文系列，发布在公众号/视频号等公开渠道。**核心 IP 不是"会写"，而是"会改"——99% 的财经文章在违反本 skill 的事实核查标准**。本 Skill 采用单 Agent 顺序执行四阶段流程。

参考样本：`reports/Tencent/《看懂腾讯》/`
...
| 06 | 用巴菲特方式拆 X 的财报 | 财务深度：毛利率/FCF（自由现金流）/ROE（净资产收益率）/SBC（股权激励费用） | 8,000-10,000 |
| 07 | `<管理层金句>`——X 的管理层值不值得托付 | 资本配置纪律 + 诚信检验 + 接班人 | 8,000-10,000 |
| 08 | 多少钱值得买，什么信号必须卖（系列终章） | DCF（现金流折现）三情景 + 红线清单 + 仓位框架 | 10,000-12,000 |
...
2. **第三方测算 MAU（月活）/份额**：QuestMobile/七麦/CBNData 等口径差异巨大（同一时点能差 2-3 倍）。**只用最可信的两个对比作 anchor，其他做定性描述**。
...
```
□ 1. 跨篇数字一致性：总市值、Non-IFRS（非国际财务报告准则）净利润、关键持股 % 全系列对齐
□ 2. 口径标注：Non-IFRS / GAAP（美国通用会计准则） / Non-IFRS-SBC / FCF（自由现金流）各用哪个，全文清楚
□ 3. 重复加计扫描：已并表子公司不在"投资组合"里、SOTP（分部加总估值）不双算
□ 4. 横向比较公平性：不能"主业 PE（市盈率）（剔除现金+组合）" vs "对手 PE（不剔）"
□ 5. 概率加权全删：见上一条
...
- 总股本会反弹：SBC（股权激励费用）集中年初授予会让股本短期上升

---

## 五、执行流程（四阶段，单 Agent 顺序执行）

### 阶段 1：调研（写 01-02 篇前完成）

1. 阅读公司近 5 年年报、最新季报（使用 WebSearch 和 WebFetch 获取）
2. 阅读至少 3 份独立卖方研报（找共识 + 反共识）
3. 用 investment-team 或 investment-research skill 先生成内部研究底稿
4. 使用 WebSearch 中英文搜索，至少3-5次不同关键词组合
5. 与用户确认 8 篇的核心论点（避免写完才发现方向不对）

### 阶段 2：写作（按 01→08 顺序写，不跳）

- 每篇写完先存 `reports/{TICKER}/《看懂{TICKER}》/0X-XX.md`（TICKER为英文名/股票代码，禁止中文）
- 不立即推 GitHub——等用户审阅
- 用户提修订意见后修改
- 修订完才 git push
...
- 改了总市值 → 全系列联动改 PE（市盈率） / 主业 PE / 折让 / FCF Yield（自由现金流收益率）
...
- **呈现正反两面**——每个核心判断附带反面论据
- **对不确定诚实**——不确定或数据不足时直接说明

---

## 一句话总结

**写《看懂 X 系列》的核心能力不等于写得好，而是改得严**——
