...
description: 财报精读：一手资料深度解读。直接解读原始财报（10-K/年报/季报/电话会纪要），关注管理层语气、附注隐藏信息、现金流质量。当用户提到"财报分析"、"earnings review"、"季报解读"、"年报分析"时使用。
...
本Skill采用单Agent顺序执行，从资料收集到报告输出，一步一验证。
...
使用 WebSearch 和 WebFetch 依次获取以下原始材料：
...
搜索关键词建议：
- 美股：`{company} 10-K 2025 site:sec.gov`、`{company} earnings call transcript Q4 2025`
- 港股：`{company} 年报 2025 site:hkexnews.hk`
- A股：`{company} 年报 2025 site:cninfo.com.cn`

如果无法获取完整原文，使用标准数据源拼凑（美股：macrotrends+stockanalysis；港股：aastocks+macrotrends；A股：东方财富+巨潮资讯），但必须标注"非原始财报，来自第三方汇总"，且关键数据两源误差>1%须标记。
...
将报告写入 `reports/{TICKER}/{TICKER}-earnings-{期间}.md`，例如 `reports/Tencent/Tencent-earnings-2025Q4.md`
